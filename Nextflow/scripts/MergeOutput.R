...
#  rownames(t_quant) <- gsub(", ",";", t_rownames)
  ...
  quant_out[[file]] <- cbind(proteins=rownames(t_quant),t_quant[,keep_columns_all])
  quant_pep_out[[file]] <- cbind(mod_peptides=rownames(t_pep_quant),t_pep_quant[,keep_pep_columns_all])
